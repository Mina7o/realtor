"""Compare list prices to market value and find deals."""
import sys
...
def find_deals(conn, min_discount=None, max_premium=None):
    ...
    return conn.execute(f"""
        ...
        FROM (
            ...
            WHERE (l.listing_status IS NULL OR l.listing_status NOT IN ('REMOVED','SOLD'))
            GROUP BY p.id
        ) d
        LEFT JOIN (
            SELECT listing_id,
...
def show_deals(rows, min_discount, max_premium):
    ...
    def print_group(label, items):
        ...
        print(f"  {'Address':42s} {'Price':>9s} {'Mkt Val':>9s} {'Diff':>7s} "
              f"{'Src':<5s} {'Chg':<7s} {'Owner':25s} {'Sources':20s} {'Flags':15s}")
        print(f"  {'-'*42} {'-'*9} {'-'*9} {'-'*7} {'-'*5} {'-'*7} {'-'*25} {'-'*20} {'-'*15}")
        for r in items:
            ...
            srcs = (r['sources'] or '')[:18]
            print(f"  {addr:40s} ${r['best_price']:>8,.0f} ${r['market_value']:>8,.0f} "
                  f"{r['diff_pct']:>+6.1f}% {r['value_source']:<5s} {chg:<7s} {owner:25s} {srcs:20s} {flags:15s}")
        print()
