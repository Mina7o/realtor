...
from app.api.common import COALESCE_MV, PROJECT_ROOT, get_deal_tier
...
def build_listing_filters(args):
    ...
    if owner_q:
        clauses.append(
            "(a.owner_name LIKE :owner1 OR mp.full_owner_name LIKE :owner2 OR up.curr_name1 LIKE :owner3 OR yp.owner1 LIKE :owner4)"
        )
    ...
    if min_acres:
        clauses.append(
            "(pd.lot_sqft >= :acres_sqft OR mp.num_totalac >= :acres_mp OR a.lot_acres >= :acres_attom OR yp.gis_acres >= :acres_york)"
        )
...
@listings_bp.route("/api/listings")
def get_listings():
    conn = get_conn()
    ...
    if request.args.get("format") == "csv":
        ...
        buf = io.StringIO()
        if results:
            w = csv.DictWriter(buf, fieldnames=results[0].keys())
            w.writeheader()
            w.writerows(results)
        else:
...
@listings_bp.route("/api/price-history/<int:listing_id>")
...
def get_charts():
    ...
    total_potential_equity = sum(d["market_value"] - d["list_price"] for d in deals_list if d["diff_pct"] < 0)
    undervalued_list = [d for d in deals_list if d["diff_pct"] < 0]
    avg_discount_pct = sum(d["diff_pct"] for d in undervalued_list) / len(undervalued_list) if undervalued_list else 0
    ...
    return jsonify({
        ...
        "portfolio": {
            "total_equity": int(total_potential_equity),
            "avg_discount": round(avg_discount_pct, 1),
            "deal_count": len(undervalued_list),
        },
    })


@listings_bp.route("/api/nearby-deals")
def get_nearby_deals():
    lat = request.args.get("lat", type=float)
    lng = request.args.get("lng", type=float)
    radius_miles = request.args.get("radius", 2.0, type=float)
    ...
    query = f"""
        SELECT l.id, p.address, c.name as county, p.lat, p.lng,
               l.list_price, {COALESCE_MV} as market_value,
               ph_old.id as ph_id
        FROM listings l
        JOIN properties p ON l.property_id = p.id
        LEFT JOIN tax_records t ON p.id = t.property_id
        ...
        WHERE p.lat BETWEEN :lat_min AND :lat_max
          ...
          AND l.list_price < market_value
        ORDER BY (l.list_price - market_value) / market_value ASC
        LIMIT 15
    """

    params = {
        "lat_min": lat - deg_range, "lat_max": lat + deg_range,
        "lng_min": lng - deg_range, "lng_max": lng + deg_range,
    }
    ...
    for r in rows:
        d = dict(r)
        d["diff_pct"] = round((d["list_price"] - d["market_value"]) * 100.0 / d["market_value"], 1)
        d["price_drop"] = bool(d["ph_id"])
        results.append(d)
...
@listings_bp.route("/api/landandfarm-listings")
def get_landandfarm_listings():
    conn = get_conn()
    limit = request.args.get("limit", 100, type=int)
    offset = request.args.get("offset", 0, type=int)
    ...
    count = conn.execute(f"SELECT COUNT(*) FROM landandfarm_listings WHERE {where}", params).fetchone()[0]
    order = "acres DESC"
    if sort == "acres_asc":
        order = "acres ASC"
    elif sort == "price":
    ...
    rows = conn.execute(f"""
        SELECT l.pid, l.county, l.title, l.address, l.price, l.acres, l.description, l.url, l.lat, l.lng,
               COALESCE(s.score_total, 0) as score_total,
               COALESCE(s.score_tier, '') as score_tier,
               COALESCE(s.score_acreage, 0) as score_acreage,
               COALESCE(s.score_ppa, 0) as score_ppa,
               COALESCE(s.score_county, 0) as score_county,
               COALESCE(s.score_cluster, 0) as score_cluster,
               COALESCE(s.score_keywords, 0) as score_keywords,
               COALESCE(s.cluster_size, 1) as cluster_size
        FROM landandfarm_listings l
        LEFT JOIN landandfarm_scores s ON l.pid = s.pid
        WHERE {where}
        ORDER BY {order}
        LIMIT :lim OFFSET :off
    """, params).fetchall()
    cols = ["pid", "county", "title", "address", "price", "acres", "description", "url", "lat", "lng",
            "score_total", "score_tier", "score_acreage", "score_ppa", "score_county", "score_cluster",
            "score_keywords", "cluster_size"]
    listings = [dict(zip(cols, r)) for r in rows]
    conn.close()
    return jsonify({"listings": listings, "total": count, "limit": limit, "offset": offset})


@listings_bp.route("/api/landandfarm-stats")
def get_landandfarm_stats():
    conn = get_conn()
    stats = conn.execute("""
        SELECT
            COUNT(*) as total,
            ROUND(SUM(acres)) as total_acres,
            ROUND(AVG(acres), 1) as avg_acres,
            ROUND(AVG(price), 0) as avg_price,
            ROUND(MIN(price), 0) as min_price,
            ROUND(MAX(price), 0) as max_price,
            ROUND(SUM(CASE WHEN lat IS NOT NULL THEN 1 ELSE 0 END)) as geocoded
        FROM landandfarm_listings
    """).fetchone()
    by_county = conn.execute("""
        SELECT county, COUNT(*) as cnt, ROUND(SUM(acres)) as total_ac,
               ROUND(AVG(price), 0) as avg_price
        FROM landandfarm_listings
        GROUP BY county
    """).fetchall()
